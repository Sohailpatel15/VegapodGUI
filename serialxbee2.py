import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QTableWidget, QTableWidgetItem, QLCDNumber, QLabel, QPushButton, QVBoxLayout, QWidget, QComboBox
...
class SerialThread(QThread):
    ...
    def send_array(self, array):
        ...
        self.ser.write(array_string.encode() + b"\n")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.serial_thread = SerialThread("/dev/ttys003")
        self.serial_thread.data_received.connect(self.update_table_rows)
        self.serial_thread.start()

    def start_timer(self):
        self.timer = QTimer()
        self.timer.timeout.connect(self.serial_thread.send_data)
    ...
    def button1_clicked(self):
        ...
        self.serial_thread.send_array(data1)
    ...
    def update_table_rows(self, data):
        ...
        if chunks[0] == "START" and chunks[-1] == "STOP":
            values = chunks[1:-1]
            for i in range(min(len(values), 42)):
...
if __name__ == '__main__':
    ...
    timer = QTimer()
    #timer.timeout.connect(lambda: self.serial_thread.send_array(data1))
    timer.start(100)
    window = MainWindow()
    window.show()
    sys.exit(app.exec_())
